...
sub create_vm {
    ...
    return do_http_post_request( "http://$test_host/lml/restricted/vm-create.pl", "name=$vm_host&esx_host=$esx_host&username=$username&expiration=$expiration_date&folder=$folder&force_boot_target=$force_boot_target" );
}
...
sub download_qr_code {
    ...
    report_progress("Downloading QR code from $url");
    my $uuid = shift;
